...
module.exports = elmongo = function (schema, options) {
    ...
    schema.on('middlewareUpdate', function (data, opts) {        
        if (opts)
            options = opts;

        this.methods.customIndex(options, data.toObject(options.serializeOptions));
    })
}
...
function customIndex (options, data) {

    var self = this,
        unindexUri = helpers.makeDocumentUri(options, data)
